...
var NarrativeConstraints = `【绝对格式要求】你必须以小说叙事风格输出。严禁使用括号、方括号、【】、冒号加引号等任何剧本标记。对话必须使用引号，并明确说话者（如“某某说”、“某某喊道”）。动作描写必须自然融入段落。
...
func LoadConstraints(path string) (string, error) {
	if path == "" {
		return NarrativeConstraints, nil
	}
	data, err := os.ReadFile(path)
	if err != nil {
		return "", fmt.Errorf("读取约束文件失败：%w", err)
	}
...
func BuildPrompt(contract *domain.Contract, currentState map[string]any, history []domain.HistoryEntry, currentMemories []string, input string, constraints string) string {
	...
	return RenderPrompt(PromptTemplate{
		...
		History:     history,
		Memories:    memories, // 关键：使用当前记忆，而非契约初始记忆
		UserInput:   input,
